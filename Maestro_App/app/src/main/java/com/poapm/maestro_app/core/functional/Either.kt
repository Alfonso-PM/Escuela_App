package com.poapm.maestro_app.core.functional
...
fun <L, R> Either<L, R>.getOrThrow(): R =
    when (this) {
        is Either.Left -> throw IllegalStateException("Either is a LEFT value")
        is Either.Right -> b
    }

fun <L, R> Either<L, R>.leftOrElse(value: L): L =
    when (this) {
        is Either.Left -> a
...
fun <L, R> Either<L, R>.leftOrThrow(): L =
    when (this) {
        is Either.Left -> a
        is Either.Right -> throw IllegalStateException("Either is a RIGHT value")
    }
